...
namespace sp{
            ...
            for (int i = 0; i < m_stack.size(); i++) {
                ...
                for (int i = 0; i < m_dim; i++) {
                    if (m_size == 0) {
                        ...
                        m_rect(i, 0) = (TYPE)std::min(data[i], m_rect(i, 0));
                        m_rect(i, 1) = (TYPE)std::max(data[i], m_rect(i, 1));
                    }
                }
    ...
    private:
        ...
        SP_REAL normData(const TYPE *data, const TYPE *base) const {
            SP_REAL norm = 0.0;
            for (int i = 0; i < m_dim; i++) {
                norm += (data[i] - base[i]) * (data[i] - base[i]);
            }
            return sqrt(norm);
        }
        ...
        SP_REAL normRect(Mem2<TYPE> &rect, const TYPE *base) const {
            ...
            for (int i = 0; i < m_dim; i++) {
                ...
                norm += (rect(i, t) - base[i]) * (rect(i, t) - base[i]);
            }
